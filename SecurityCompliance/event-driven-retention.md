...
Период хранения содержимого часто основан на возрасте содержимого. Например, документы могут храниться в течение семи лет после создания, а затем их удаляют. Но с помощью меток хранения в Office 365 можно также начать отсчет периода хранения c момента возникновения события определенного типа. Ко всему содержимому с меткой хранения, присвоенной для события соответствующего типа, применяются действия по хранению, определенные в метке.
...
В центре соответствия требованиям Microsoft 365, центре безопасности Microsoft 365 или центре безопасности и соответствия требованиям Office 365 в области навигации слева выберите **Классификации** > **Метки**  >  вкладка**Метки хранения** > **Создать метку**.
...
После присвоения метки содержимому можно выполнять поиск по содержимому, чтобы находить все содержимое, классифицированное с определенной меткой или содержащее определенный идентификатор ресурса.
...
Кроме того, с помощью сценария PowerShell можно автоматизировать хранение на основе событий из бизнес-приложений. Ниже перечислены командлеты PowerShell, доступные для хранения на основе событий.
