...
ms.locfileid: "32262311"
---
# <a name="controlling-outbound-spam-in-office-365"></a>Контроль за исходящим спамом в Office 365
...
> [!IMPORTANT]
> Уведомление, когда отправитель ограничено, теперь является частью платформы оповещений _Амп_ Security Center (SCC). Вместо методов, описанных ниже, для отправки уведомления список пользователей для оповещения можно найти в оповещении "пользователь запретил отправку электронной почты". Чтобы настроить оповещение, откройте [страницу политики оповещений](https://sip.protection.office.com/alertpolicies) в центре безопасности _Амп_ соответствие требованиям, так как в будущем метод будет удален. Узнайте о новом интерфейсе " [ограниченНые пользователи](https://docs.microsoft.com/en-us/Office365/SecurityCompliance/removing-user-from-restricted-users-portal-after-spam)". "
...
а. **Отправьте массовые сообщения электронной почты через собственные локальные почтовые серверы**. Это означает, что клиент должен поддерживать собственную инфраструктуру электронной почты для этого типа электронной почты.
...
[Пример уведомления, когда отправитель блокирует отправку исходящих нежелательных сообщений](sample-notification-when-a-sender-is-blocked-sending-outbound-spam.md)
