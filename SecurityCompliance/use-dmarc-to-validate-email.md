...
- **Адрес "mail from"**: идентифицирует отправителя и указывает, куда отправлять уведомления о возврате при возникновении проблем с доставкой сообщения, например уведомления о недоставке. Он задан в конверте сообщения и обычно не отображается почтовым приложением. Иногда это называется адресом 5321. MailFrom или адресом обратного пути.
    
- **"От" адрес**: адрес, отображаемый почтовым приложением как адрес отправителя. Он отображен в почтовом приложении как адрес отправителя. Этот адрес указывает автора сообщения, вернее, почтовый ящик человека или систему, где оно было создано. Он иногда зовется адресом 5322.From.
...
- *domain* — домен, который нужно защитить. По умолчанию запись защищает почту из домена и всех его поддоменов. Например, если задано \_значение DMARC.contoso.com, DMARC защищает почту от домена и всех поддоменов, таких как housewares.contoso.com или plumbing.contoso.com. 
