...
- **Адрес «mail From»**: идентифицирует отправителя и указывает, где можно отправлять возвращаемое уведомления в случае возникновения проблем с доставки сообщений, например уведомлений о доставке. Отображается в правом конверт сообщения электронной почты и обычно не отображается в приложении электронной почты. Это иногда называется 5321.MailFrom адрес или адрес обратного пути.
...
- *домен* — это домен, который необходимо защитить. По умолчанию запись защищает почты из домена и всех поддоменов. Например, если указать \_dmarc.contoso.com, а затем DMARC защищает почты из домена и все дочерние домены, например housewares.contoso.com или plumbing.contoso.com. 
    
- *TTL* всегда должно быть эквивалент один час. Единицы измерения TTL, либо часов (1 час) минут (60 минут), или секунд (3600 секунд) будут зависеть от регистратора для вашего домена. 
    
- *процент = 100* указывает, что это правило следует использовать для 100% сообщений электронной почты.
    
- *Политика* определяет, какие политики принимающий сервер для подписки в случае сбоя DMARC. Можно задать политику для нет, в карантин, или отклонить. 
