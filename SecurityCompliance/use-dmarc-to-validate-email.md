...
- **"От" адрес**: адрес, отображаемый почтовым приложением как адрес отправителя. Этот адрес определяет автора сообщения электронной почты. Это значит, что почтовый ящик человека или системы отвечает за написание сообщения. Иногда это называется адресом 5322. from.
...
- *домен* — это домен, который вы хотите защитить. По умолчанию запись защищает почту от домена и всех поддоменов. Например, если задано \_значение DMARC.contoso.com, DMARC защищает почту от домена и всех поддоменов, таких как housewares.contoso.com или plumbing.contoso.com. 
    
- Значение *TTL* всегда должно быть равно одному часу. Единица, используемая для TTL (1 час), минут (60 минут) или секунд (3600 секунд), зависит от регистратора для вашего домена. 
    
- *PCT = 100* означает, что это правило должно использоваться для 100% электронной почты.
    
- *Политика* указывает, какая политика должна выполнять принимающий сервер при отказе DMARC. Для политики можно установить значение None, карантин или отклонить. 
