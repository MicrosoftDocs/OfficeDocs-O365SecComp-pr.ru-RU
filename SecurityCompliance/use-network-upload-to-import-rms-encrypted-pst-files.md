...
- В этой процедуре выполняется копирование и сохранение копии ключа шифрования, ключа к хранилищу данных и нескольких ключей и URL-адресов, служащих для идентификации. Эти сведения будут использоваться на шаге 5 для шифрования и отправки PST-файлов. Необходимо защитить эти ключи так же, как вы защищаете пароли и другую информацию, связанную с обеспечением безопасности. Например, можно сохранить эти ключи в документе Microsoft Word, защищенном паролем, или на зашифрованном USB-накопителе. Примеры таких ключей, идентификаторов и URL-адресов см. в разделе [Дополнительные сведения](#more-information). 
...
- [Активация службы управления правами Azure](#activating-azure-rms)
    
- [Настройка службы управления правами в Exchange Online](#configuring-rms-in-exchange-online)
    
- [Установка клиента службы управления правами Active Directory](#installing-the-active-directory-rms-client)
...
Сохраните модуль Azure Active Directory для Windows PowerShell, чтобы он открывался и подключился к службе Microsoft Online. В следующем действии вам понадобится выполнить команду в этом окне.
...
Следующий шаг — получение идентификатора клиента и URL-адреса расположения лицензирования для службы Azure RMS в Организации. Скопируйте и сохраните эти сведения в тот же файл, который содержит симметричный ключ из действия 2. ИДЕНТИФИКАТОР и URL-адрес будут использоваться на шаге 5 для шифрования PST-файлов.
...
7. После отображения URL-адреса скопируйте его и сохраните в файле, в который вы сохранили другие разделы. Убедитесь, что вы скопировали весь URL-адрес. 
...
После выполнения шага 1 (шаг 4) вы можете использовать средство O365ImportTool. exe для шифрования и отправки PST-файлов в Office 365. Это средство шифрует PST-файлы, а затем отправляет их и сохраняет в хранилище Azure в облаке Майкрософт. Чтобы выполнить это действие, необходимо, чтобы PST-файлы хранились в файловом ресурсе или на файловом сервере в вашей организации. В описанной ниже процедуре такое расположение называется исходным каталогом. При каждом запуске средства O365ImportTool.exe можно будет указать другой исходный каталог. 
...
3. Выполните следующую команду, чтобы зашифровать и отправить PST-файлы в Office 365.
    ...
    > [!TIP]
    > После успешного выполнения команды O365ImportTool.exe и проверки правильности всех параметров сохраните синтаксис командной строки в тот же (защищенный) файл, в котором вы сохранили сведения, полученные в предыдущих действиях. После этого вы можете скопировать и вставить эту команду в командную строку каждый раз, когда необходимо запустить средство O365ImportTool. exe для шифрования и отправки PST-файлов в Office 365. Можно изменить только те значения, которые необходимы для параметров `/srcdir:` и. `/upload-dest:` 
...
6. Щелкните правой кнопкой мыши элемент **ingestiondata** и выберите пункт **Открыть редактор контейнеров BLOB-объектов**.
...
Последний шаг — создание задания импорта PST-файлов в службе импорта в Office 365. Как было сказано выше, вам необходимо отправить файл сопоставления для импорта PST-файлов, созданный в действии 7. После создания нового задания служба импорта будет использовать сведения, приведенные в файле сопоставления, для отмены шифрования и импорта PST-файлов (которые были отправлены в Office 365 на шаге 5) в указанный почтовый ящик пользователя. 
...
9. Когда имя CSV-файла появится в списке, выберите его и щелкните **Проверить**, чтобы проверить CSV-файл на наличие ошибок.  
    ...
    > [!NOTE]
    > Как описано выше, при шифровании PST-файлов добавляется `.pfile` расширение имени PST-файла. Необходимо добавить `.pfile` расширение к имени PST-файлов в CSV-файле. Если вы не сделаете этого, CSV-файл не пройдет проверку. 
...
- Зачем импортировать PST-файлы в Office 365?
  ...
  - Это обеспечивает защиту от потери данных. PST-файлы, импортированные в почтовые ящики Office 365, наследуют функции высокого уровня доступности Exchange Online, а не хранение данных на компьютере пользователя.
