...
С помощью подчиненных меток вы можете группировать одну или несколько меток под верхним колонтитулом, которые видны пользователю в приложении Office. Например, в разделе "Конфиденциально" ваша организация может использовать несколько разных меток для определенных типов этой классификации. В этом примере метка "Конфиденциально" — это простая текстовая подпись без параметров защиты. Она имеет подчиненные метки, поэтому ее нельзя применять к контенту. Вместо этого пользователям необходимо выбрать раздел "Конфиденциально", чтобы просмотреть подчиненные метки, после чего они смогут выбрать подчиненную метку для применения к контенту.
