...
---
# <a name="message-trace-in-the-security--compliance-center"></a>Трассировка сообщений в центре безопасности _Амп_ соответствие требованиям
...
3. В нижней левой панели навигации разверните узел **центры администрирования** и выберите **соответствие требованиям безопасности _амп_**.

4. На открывшейся странице **соответствие требованиям безопасности _амп_** разверните пункт **почтовый ящик**и выберите **Трассировка сообщений**.
...
Вы также можете ввести адреса электронной почты внешних отправителей и получателей. Поддерживаются подстановочные знаки`*@contoso.com` ( `scot?@contoso.com`или), но нельзя одновременно использовать несколько подстановочных знаков в одном и том же поле.
...
- **События сообщений**: в этом разделе содержатся классификации, помогающие классифицировать действия, которые служба использует в сообщениях. Ниже перечислены некоторые интересные события, которые могут возникнуть.
   ...
   - **Разрешено**: сообщение было перенаправлено на новый адрес получателя на основе поиска в Active Directory. В этом случае исходный адрес получателя перемещается в отдельную строку в трассировке сообщения вместе с итоговым состоянием доставки сообщения.
...
- **Дополнительные сведения**: в этом разделе содержатся следующие сведения:
   ...
   - **Идентификатор сообщения**: это значение описывается в разделе " [идентификатор сообщения](#message-id) " ранее в этой статье. Пример: `<d9683b4c-127b-413a-ae2e-fa7dfb32c69d@DM3NAM06BG401.Eop-nam06.prod.protection.outlook.com>`.
...
- **РеЦипиент_статус**: состояние доставки сообщения получателю. Если сообщение было отправлено нескольким получателям, он покажет всем получателям и соответствующее состояние каждого из них в \<формате:*состояние*\> *адреса*\>##\<электронной почты. Например:
...
- **message_id**: это значение описывается в разделе " [идентификатор сообщения](#message-id) " ранее в этой статье. Пример: `<d9683b4c-127b-413a-ae2e-fa7dfb32c69d@DM3NAM06BG401.Eop-nam06.prod.protection.outlook.com>`.
...
- **саурце_контекст**: Дополнительные сведения, связанные с **исходным** полем. Например:
...
- **реЦипиент_аддресс**: адреса электронной почты получателей сообщения. Несколько адресов электронной почты разделяются символом точки с запятой (;).
...
- **ссылка**: в этом поле содержатся дополнительные сведения о конкретных типах событий. Например:
...
|`DI=SN`|Сообщение было перенаправлено через пул доставки с более высоким уровнем риска. Более подробную информацию можно найти в разделе [пул доставки высокого риска для исходящих сообщений](https://technet.microsoft.com/library/jj200746.aspx).|
...
|**Значение**|**Описание**|
|:-----|:-----|
|`AMA=SUM|v=1|`также`AMA=EV|v=1`|Сообщение было определено как содержащее вредоносные программы. `SUM` указывает, что вредоносная программа могла быть обнаружена любым числом ядер. `EV` указывает на наличие вредоносных программ, обнаруженных определенным ядром. При обнаружении антивирусной программой последующих действий включается.|
...
|`Action=<ActionDefinition>`|Действие, которое было применено. Список доступных действий представлен [в статье действие правил обработки почты в Exchange Online](https://technet.microsoft.com/library/jj919237.aspx).|
