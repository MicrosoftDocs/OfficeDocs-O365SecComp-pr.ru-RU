...
> [!NOTE]Вы также можете ввести адреса электронной почты внешних отправителей и получателей. Поддерживаются подстановочные знаки`*@contoso.com` ( `scot?@contoso.com`или), но нельзя одновременно использовать несколько подстановочных знаков в одном и том же поле.
> [!NOTE]Можно вставить несколько отправителей или список получателей, разделенных точкой с`;`запятой (). Допускается использование`\s`пробелов (),`\r`символов возврата каретки (`\n`) и последующих строк ().
