...
- **Шаг 1 (необязательно)** - [Настройка групп для контроля (необязательно)](#step-1---set-up-groups-for-supervision-optional)
    ...
    Прежде чем приступать к надзору, определите, кому требуется проверить связь и кто будет выполнять обзоры. Если вы хотите начать работу с несколькими пользователями, чтобы узнать, как работает функция контроля, вы можете пропустить настройку групп.
...
- **Шаг 5 (необязательно)** [Проверка политики контроля](#step-5---test-your-supervision-policy-optional)
...
- **Шаг 6 (необязательно)** [Настройка Outlook для рецензентов, которые не хотят использовать панель мониторинга Office 365 или Outlook в Интернете (прежнее название — Outlook Web App) для просмотра контролируемого обмена данными](#step-6---configure-outlook-for-reviewers-optional)
...
Используйте текстовый редактор (например, "Блокнот"), чтобы создать файл, включающий ключевые слова, которые необходимо отслеживать в политике контроля. Убедитесь, что каждый термин находится в отдельной строке, и сохраните файл в формате **Юникод/UTF-16 (с прямым порядкОм байтов)** .
...
9. В случае успеха вы увидите папку " **надзор" \<(имя\> политики)** , указанную в представлении списка папок в Outlook.
