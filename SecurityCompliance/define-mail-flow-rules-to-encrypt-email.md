...
Локальный пользователи могут отправлять зашифрованные почты с помощью OME, если вы направляете электронной почты через Exchange Online. Для этого необходимо настроить почту в поток с сервера электронной почты в Office 365. Как только вы настроили почты через Office 365, можно сделать правила потока почты для OME с помощью этой статьи.
