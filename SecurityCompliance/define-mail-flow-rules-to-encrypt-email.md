...
## <a name="hybrid-exchange-environments-do-this-first"></a>Гибридные среды Exchange: первый шаг
Локальный пользователи могут отправлять зашифрованные почты с помощью OME, если вы направляете электронной почты через Exchange Online. Для этого необходимо настроить почту для потока с сервера электронной почты в Office 365. Один раз вы настроили почты через Office 365, а затем можно сделать правила потока почты для OME с помощью этой статьи.

Сведения содержатся в разделе [Настройка соединителей для маршрутизации почты между Office 365 и серверы электронной почты](https://docs.microsoft.com/exchange/mail-flow-best-practices/use-connectors-to-configure-mail-flow/set-up-connectors-to-route-mail). В частности, выполните действия, описанные в «часть 2: Настройка почты для потока с сервера электронной почты в Office 365».
