...
---
# <a name="create-a-custom-sensitive-information-type"></a>Создание пользовательского типа конфиденциальных данных
...
Если вам необходимо обнаруживать и защищать конфиденциальные сведения другого типа, например идентификаторы сотрудников, для которых в вашей организации используется особый формат, вы можете создать пользовательский тип конфиденциальных данных. Для определения типа конфиденциальных данных используется XML-файл, который называется пакетом правил.
...
Создав XML-документ правильного формата, вы можете отправить его в Office 365 с помощью PowerShell. Затем вы сможете использовать этот пользовательский тип конфиденциальных данных в своих политиках защиты от потери данных и проверить, обнаруживает ли система конфиденциальные данные так, как вы задумали.
...
В связи с разнообразием сред клиентов и требований к совпадению содержимого служба поддержки Майкрософт не может помочь в определении совпадений содержимого, например определении пользовательских классов или шаблонов регулярных выражений (RegEx). В вопросах разработки, тестирования и отладки совпадений содержимого пользователям Office 365 придется рассчитывать только на внутренние ИТ-ресурсы. Кроме того, можно использовать внешние консультационные ресурсы, такие как Консультационные службы Майкрософт (MCS). Специалисты службы поддержки могут в некоторой степени помочь настроить функцию, но они не гарантируют, что разработка в области совпадений содержимого будет соответствовать требованиям или обязательствам клиента. Примером такой возможной поддержки может служить предоставление шаблона регулярного выражения во время тестирования. В службе поддержки также могут помочь устранить неполадки в уже имеющемся шаблоне, который не срабатывает должным образом, на одном конкретном примере сравнения содержимого.
 ...
 Дополнительные сведения о модуле RegEx в .NET, используемом для обработки текста, см. в документации по [регулярным выражениям в .NET](https://docs.microsoft.com/ru-RU/dotnet/standard/base-types/regular-expressions).
...
2. [Подключитесь к Центру безопасности и соответствия требованиям Office 365 с помощью удаленного сеанса PowerShell](http://go.microsoft.com/fwlink/?LinkID=799771&amp;clcid=0x409).
    
3. В Центре безопасности и соответствия требованиям введите команду New-DlpSensitiveInformationTypeRulePackage -FileData (Get-Content -Path "C:\путь\НовыйПакетПравил.xml" -Encoding Byte).
    
    Обязательно используйте расположение файла, в котором фактически хранится ваш пакет правил. Путь C:\путь\ — это просто заполнитель.
    
4. Для подтверждения введите Y и нажмите клавишу ВВОД.
    
5. Убедитесь, что ваш новый тип конфиденциальных данных отправлен. Для этого введите команду Get-DlpSensitiveInformationType, чтобы отобразить список всех типов конфиденциальных данных. Вы можете быстро отделить пользовательские типы конфиденциальных данных от встроенных, взглянув на столбец "Издатель". Отфильтруйте список для определенного типа конфиденциальных данных, выполнив команду Get-DlpSensitiveInformationType -Identity "имя типа конфиденциальных данных".
...
2. В PowerShell в Центре безопасности и соответствия требованиям выполните одно из указанных ниже действий.
    
  - Чтобы удалить весь пакет правил и все содержащиеся в нем объекты, выполните указанные ниже действия.
    
    Введите команду Remove-DlpSensitiveInformationTypeRulePackage "ИмяВашегоПакетаПравил". Для приведенного выше примера XML следует ввести Remove-DlpSensitiveInformationTypeRulePackage "Employee ID Custom Rule Pack".
    
    Учтите следующее: чтобы идентифицировать ваш пакет правил, можно использовать элемент \<Name\> (для любого языка) в элементе \<Rule Pack\> или GUID атрибута id элемента RulePack.
    
  - Чтобы удалить один объект из пакета правил, выполните указанные ниже действия.
    
    Отправьте новую версию пакета правил, в котором удален соответствующий объект, выполнив команду Set-DlpSensitiveInformationTypeRulePackage. Прежде чем удалять тип конфиденциальных данных, убедитесь, что на него больше не ссылается ни одна политика защиты от потери данных или правило транспорта.
    
3. Для подтверждения введите Y и нажмите клавишу ВВОД.
    
4. Убедитесь, что новое правило удалено, выполнив команду Get- DlpSensitiveInformationType. В отображенном списке не должно быть имени вашего типа конфиденциальных данных.
