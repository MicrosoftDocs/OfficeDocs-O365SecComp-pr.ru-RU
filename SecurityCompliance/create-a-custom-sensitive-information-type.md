...
Если вам нужно обнаруживать и защищать конфиденциальные сведения другого типа (например, идентификаторы сотрудников или номера проектов, для которых в вашей организации используется особой формат), вы можете создать пользовательский тип конфиденциальной информации.
...
Создать пользовательский тип конфиденциальной информации для защиты от потери данных в Центре безопасности и соответствия требованиям Office 365 можно следующими способами:
...
|:-----|:-----|
|Название и описание на одном языке|Поддерживается несколько языков для имени и описания|
|Поддерживается один шаблон (основной).|Поддерживается несколько шаблонов помимо основного.|
...
|Можно настраивать уровень вероятности для типа конфиденциальной информации.|Уровень вероятности настраивается для типа конфиденциальной информации и каждого шаблона в отдельности.|
...
- Для работы с типами конфиденциальной информации вы должны быть знакомы с регулярными выражениями (RegEx). Дополнительные сведения о модуле RegEx .NET, используемом для обработки текста, см. в статье [Регулярные выражения в .NET](https://docs.microsoft.com/dotnet/standard/base-types/regular-expressions).
...
3. На открывшейся странице **Требования для сопоставления** нажмите **Добавить элемент** и настройте следующие параметры:

  - **Обнаруживать контент, содержащий**:
 
    А. Нажмите **Любой из этих элементов** и выберите **Регулярное выражение**.

    Б. В поле регулярного выражения введите `(\s)(\d{9})(\s)` (девятизначное число с пробелами с обеих сторон).
  
  - **Вспомогательные элементы**: нажмите **Добавить вспомогательные элементы** и выберите **Содержит этот список ключевых слов**.
...
3. На открывшейся странице отправьте документ на проверку, перетащив файл или нажав кнопку **Обзор** и выбрав файл.
